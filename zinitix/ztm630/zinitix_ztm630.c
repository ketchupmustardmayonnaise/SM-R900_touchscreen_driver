...
#endif
#include "zinitix_ztm630.h"

//#define CONFIG_DRV_SAMSUNG
...
static s32 ztm620_read_data(struct i2c_client *client, u16 reg, u8 *values, u16 length)
{
	...
	if (info->debug_flag == 1) {
		...
		pr_info("[sec_input] i2c_cmd: W: 0x%04X | R:", reg);
		for (i = 0; i < length; i++)
			pr_cont("0x%02X ", values[i]);
		pr_cont("\n");
	}
	return length;
}
...
static s32 ztm620_write_cmd(struct i2c_client *client, u16 reg)
{
	...
	if (info->debug_flag == 1)
		pr_info("[sec_input] i2c_cmd: W: 0x%04X\n", reg);
...
static void set_gesture_mode(struct ztm620_info *info, bool enable) 
{
...
static int ztm620_get_ic_fw_size(struct ztm620_info *info)
{
	...
	if (info->chip_code == ZTM620_IC_CHIP_CODE || info->chip_code == ZTM630_IC_CHIP_CODE)
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				dev_info(&client->dev, "%s:[%d][P] x=%3d y=%3d w=%d\n",
					__func__, i, x, y, info->wet_mode);
#else
				dev_info(&client->dev, "%s:[%d][P] w = %d\n",
...
#else
					dev_info(&client->dev, "%s:[%d][RP][0x%02x%02x%02x]\n",
							__func__, i, info->cap_info.fw_version,
							info->cap_info.fw_minor_version,
							info->cap_info.reg_data_version);
#endif
...
static ssize_t ztm620_show_touch_active(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	return scnprintf(buf, PAGE_SIZE, "%d\n", info->gesture_enable ? 0 : 1);
}
...
static DEVICE_ATTR(touch_active, S_IRUGO, ztm620_show_touch_active, NULL);
#ifdef CONFIG_TIZEN_SEC_KERNEL_ENG
				  ...
				  char *buf)
{
	...
	dev_err(&info->client->dev, "%s: enabled: %d\n", __func__, enabled);
				...
				const char *buf, size_t count)
{
	...
	if (ret < 0) {
		dev_err(&info->client->dev, "%s: failed to read buffer: %d \n", __func__, ret);
		return ret;
	}
	...
	if (enabled == 0) {
		...
		ztm620_input_open(info->input_dev);
	}
...
static struct attribute *sec_tsp_input_attrs[] = {
	&dev_attr_enabled.attr,
	NULL,
};

static struct attribute_group sec_tsp_input_attrs_group = {
	.attrs = sec_tsp_input_attrs,
};
...
static int ztm620_init_sec_factory(struct ztm620_info *info)
{
	...
	ret = sysfs_create_group(&info->input_dev->dev.kobj, &sec_tsp_input_attrs_group);
	if (unlikely(ret)) {
		dev_err(&info->client->dev, "%s:Failed to create touchscreen sysfs group\n", __func__);
	}
					...
					struct zxt_ts_platform_data *pdata)
{
	...
	pr_info("%s:model:%s, reg_boot_on:%d, x:%d, y:%d fw_name:%s, vdd_en:%d, int:%d, reset:%d\n",
...
static int ztm620_bezel_sysfs_init(struct ztm620_info *info)
{
	...
	//test
	dev_err(&info->client->dev, "%s:skipped\n", __func__);
	return ret;

#ifdef CONFIG_DRV_SAMSUNG
 	ddata->rotary_dev = sec_device_create(ddata, "bezel");
#else
	ddata->rotary_dev = device_create(sec_class, NULL, 0, info, "bezel");
#endif
